...
// import { auth } from '@/lib/firebase'; // Comment out Firebase auth
// import { onAuthStateChanged, signOut as firebaseSignOut, type User as FirebaseAuthUser } from 'firebase/auth';
import { supabase } from '@/lib/supabaseClient'; // Import Supabase client
import type { User as SupabaseAuthUser, Session } from '@supabase/supabase-js'; // Import Supabase types

// import { getUserProfile } from '@/lib/userProfileService'; // Firebase specific
// import type { UserProfile } from '@/lib/types'; // Firebase specific
...
export function AppLayout({ children }: { children: ReactNode }) {
  ...
  // const [userProfile, setUserProfile] = useState<UserProfile | null>(null); // TODO: Implement Supabase profile fetching
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      // TODO: Fetch user profile from Supabase 'profiles' table if currentUser exists
      // if (currentUser) {
      //   try {
      //     // Example: const profile = await getSupabaseUserProfile(currentUser.id);
      //     // setUserProfile(profile); 
      //   } catch (error) {
      //     console.error("Error fetching Supabase user profile in layout:", error);
      //     // setUserProfile(null);
      //   }
      // } else {
      //   // setUserProfile(null);
  ...
  // Simplified display logic for Supabase user
  const displayName = authUser?.email?.split('@')[0] || "مستخدم"; // Supabase user object might not have displayName by default
  const displayEmail = authUser?.email || "";
  const avatarUrl = authUser?.user_metadata?.avatar_url; // Common way to store avatar in Supabase
  const avatarHint = avatarUrl ? 'user provided avatar' : 'person letter';
              ...
              <div className="flex items-center gap-3 mb-3 cursor-pointer hover:bg-sidebar-accent/20 p-2 rounded-md transition-colors">
                ...
                {state === 'expanded' && (
                  <div className="flex flex-col overflow-hidden">
                    ...
                    {/* TODO: Display user level/points once Supabase profile is integrated */}
                  </div>
                )}
              </div>
            </Link>
